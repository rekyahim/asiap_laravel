        ...
        :root {
            ...
            @php
                ...
                $renderRiwayatBlock = function ($data, $is_latest) use ($sdt) {
                    ...
                    $tglPenyampaian = $data->TGL_PENYAMPAIAN
                        ? date('d M Y H:i', strtotime($data->TGL_PENYAMPAIAN))
                        : '—';
            ...
            .map-container { width: 100%; max-width: 350px; height: 180px; border-radius: 8px; overflow: hidden; margin-top: 5px; border: 1px solid #ccc; }
                    ...
                    if ($koordinatOP) {
                        ...
                        echo "<iframe width='100%' height='180' frameborder='0' style='border:0' src='https://www.google.com/maps?q=" .
                            trim($lat) .
                            ',' .
                            trim($lng) .
                            "&hl=id&z=17&output=embed'></iframe>";
                        echo '</div>';
            ...
            <tbody>
                @forelse($rows as $i => $r)
                    @php
                        ...
                        $status = isset($r->latestStatus) ? $r->latestStatus->STATUS_PENYAMPAIAN : null;

                        if ($status != null) {
                            if ($status == '1') {
                                $okPeny = '<span class="icon ok">✓</span> <span class="badge-soft">TERSAMPAIKAN</span>';
                            } else {
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center py-4 text-muted">Tidak ada data NOP yang ditemukan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
                    ...
                    <tbody>
                        @forelse($rows as $i => $r)
                            @php
                                ...
                                $status = isset($r->STATUS_PENYAMPAIAN) ? $r->STATUS_PENYAMPAIAN : null;
                                ...
                                if ($status != null) {
                                    ...
                                    $okPeny =
                                        '<span class="icon no">✕</span> <span class="badge-soft">BELUM DIPROSES</span>';
                                }
