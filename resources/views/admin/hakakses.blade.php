...
.table thead th{
    ...
    font-size:.8rem;text-transform:uppercase;color:#5d6d8a
}
.table-hover tbody tr:hover{background:#f9fbff}

/* ===== AKSI ===== */
th.col-aksi{width:260px}
.aksi-cell{
    display:inline-flex;
    gap:.45rem;
    align-items:center;
...
.btn-aksi i{font-size:1.05rem}

/* Edit - Biru */
.btn-aksi.edit{background:#5B7CFA}
.btn-aksi.edit:hover{background:#4a6cf7}

/* Reset / View - Oranye */
.btn-aksi.view{background:#FFB020}
.btn-aksi.view:hover{background:#f5a000}

/* Hapus - Merah */
.btn-aksi.delete{background:#FF8A65}
.btn-aksi.delete:hover{background:#ff7043}
</style>
@endpush
...
<td>
@if($row->TGLPOST)
{{ \Carbon\Carbon::parse($row->TGLPOST)->setTimezone($tz)->format('Y-m-d H:i') }} WIB
@else
-
@endif
</td>
