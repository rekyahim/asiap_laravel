...
@section('content')
  ...
  <style>
    ...
    .collapse-row td{ background:#fff; padding:0!important; border-bottom:1px solid var(--border); }
    .module-form-container{ padding:1.25rem 1.5rem; }
    .switch-grid{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.5rem 1rem; }
    @media (min-width:768px){ .switch-grid{ grid-template-columns:repeat(3,minmax(0,1fr)); } }
    @media (min-width:992px){ .switch-grid{ grid-template-columns:repeat(4,minmax(0,1fr)); } }
    th.aksi-head{ padding-left:var(--aksi-offset); }
    .aksi-cell{ display:inline-flex; flex-wrap:wrap; align-items:center; gap:.5rem; }
    .btn-kelola{ width:var(--aksi-offset); justify-content:center; }
  </style>
  @endpush
  ...
  <div class="container-fluid">
    <div class="card shadow-soft">
      ...
      <div class="card-body pt-3 pb-0">
        @if (session('success'))
          <div class="alert alert-success py-2"><i class="bi bi-check2-circle me-1"></i>{{ session('success') }}</div>
        @endif
        @if (session('error'))
            ...
            @forelse ($roles as $r)
              <tr>
                ...
                <td class="text-start">
                  <div class="aksi-cell">
                    {{-- Kelola Modul --}}
                    <button class="btn btn-brand btn-sm btn-kelola"
                            type="button"
                ...
                <td colspan="4">
                  <div class="collapse" id="collapse-{{ $r->ID }}">
                    <div class="module-form-container">
                     <form method="POST" action="{{ route('admin.hakakses.modul.update', ['hak' => $r->ID]) }}">
    @csrf @method('PATCH')
...
@endphp
    ...
    {{ in_array($m->ID, $owned) ? 'checked' : '' }}>
                              ...
                              <label class="form-check-label" for="m-{{ $r->ID }}-{{ $m->ID }}">
                                {{ $m->NAMA_MODUL }}
                              </label>
                            </div>
                          @endforeach
